<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>School Footer</title>
    <link rel="stylesheet" href="assets/css/footer.css">
</head>
<body>
    <footer class="footer">
        <div class="footerContainer">

            <div class="school-info">
                <div class="school-name">
                    <img src="assets/imgs/logo.jpg" alt="Lucena South II Elementary School Logo">
                    Lucena South II Elementary School
                </div>
                <div class="sub-text">
                    <p class="short-text" style="text-align: center;">"Timog Dos, Hatid ay Dunong na Lubos"</p>
                </div>
            </div>

            <div class="info-container">
                <div class="contact-section">           

                    <div class="contact-container">
                        <div class="emailNlocation">
                            <!-- Email Link -->
                            <div class="contact-item">
                                <div class="email">
                                    <a href="mailto:109732@deped.gov.ph" class="contact-link">
                                        <div class="contact-text">
                                            <div class="contact-title">109732@deped.gov.ph</div>
                                            <div class="contact-detail">Email Address</div>
                                        </div>
                                    </a>
                                </div>    
                        </div>

                            <!-- Location Link (Google Maps) -->
                            <div class="contact-item">
                                <div class="location">
                                    <a href="https://maps.app.goo.gl/94YUjoaVeRsRGcUX6" target="_blank" class="contact-link">
                                        <div class="contact-text">
                                            <div class="contact-title">Teody Street, Capitol Homesite, <br> Brgy. Cotta, Lucena, Philippines</div>
                                            <div class="contact-detail">Location</div>
                                        </div>
                                    </a>
                                </div>    
                            </div>
                        </div>
                        
                        <div class="fbNsched">
                            <!-- Facebook Link -->
                            <div class="contact-item">
                                <div class="fb">
                                    <a href="https://www.facebook.com/DepEdTayoLS2ES109732/about" target="_blank" class="contact-link">
                                        <div class="contact-text">
                                            <div class="contact-title">DepEd Tayo Lucena South II ES - Calabarzon</div>
                                            <div class="contact-detail">Visit Our Facebook Page</div>
                                        </div>
                                    </a>
                                </div>
                            </div>

                            <!-- Schedule (non-clickable) -->
                            <div class="contact-item">
                                <div class="sched">
                                    <div class="sched-container">
                                        <div class="contact-text">
                                            <div class="contact-title">Office Time Availability</div>
                                                <div class="contact-detail">
                                                    <div class="schedule-hours">
                                                        <strong>Monday - Friday</strong><br>
                                                        8am - 4pm<br>
                                                        <strong>Saturday - Sunday</strong><br>
                                                        10am - 2pm
                                                    </div>
                                                </div>    
                                        </div>
                                    </div>
                                </div>
                                <BR>
                            </div>
                        </div>
                    </div>
                    <div class="info-container-links">
                        <a href="#privacy-policy" class="info-link privacy-policy">Privacy Policy</a>
                          <span class="separator"></span>
                        <a href="#data-protection" class="info-link data-protection">Data Protection</a>
                          <span class="separator"></span>
                        <a href="#terms-of-use" class="info-link terms-of-use">Terms of Use</a>
                    </div>
                </div>
            </div>
        </div>
        <div class="copyright">
            © 2025 St. Lucena South II Elementary School. All Rights Reserved.
        </div>
    </footer>
</body>
</html>