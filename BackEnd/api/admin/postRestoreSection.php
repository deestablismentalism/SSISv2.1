...
try {
    ...
    $response = $sections->restoreSection($sectionId);
    if(!$response) {
        echo json_encode(['success'=> false,'message'=> 'Section restoration failed']);
        exit();
    }
    echo json_encode(['success'=> true,'message'=>'Section restored successfully']);
    exit();
}
