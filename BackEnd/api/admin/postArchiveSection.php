...
try {
    ...
    $response = $sections->archiveSection($sectionId);
    if(!$response) {
        echo json_encode(['success'=> false,'message'=> 'Section archiving failed']);
        exit();
    }
    echo json_encode(['success'=> true,'message'=>'Section archived successfully']);
    exit();
}
